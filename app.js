...
renderCategories()};
document.querySelector('#addTag').onclick=()=>{document.querySelector('#tagCategorySelect').innerHTML=allCategories().map(category=>`<option value="${esc(category)}">${esc(category)}</option>`).join('');const selected=selectedCategories('expenseCategory')[0];if(selected)document.querySelector('#tagCategorySelect').value=selected;document.querySelector('#tagName').value='';document.querySelector('#tagColor').value='#ed7248';document.querySelector('#tagModal').hidden=false;document.querySelector('#tagName').focus()};
document.querySelector('#closeTagModal').onclick=()=>document.querySelector('#tagModal').hidden=true;
document.querySelector('#tagForm').onsubmit=e=>{e.preventDefault();const category=document.querySelector('#tagCategorySelect').value,name=document.querySelector('#tagName').value.trim(),color=document.querySelector('#tagColor').value;if(!name)return;if((categoryTags[category]||[]).includes(name))return alert('이미 추가된 태그예요.');categoryTags={...categoryTags,[category]:[...(categoryTags[category]||[]),name]};tagColors={...tagColors,[category]:{...(tagColors[category]||{}),[name]:color}};localStorage.setItem(tagKey,JSON.stringify(categoryTags));localStorage.setItem(tagColorKey,JSON.stringify(tagColors));document.querySelector('#tagModal').hidden=true;const selected=selectedCategories('expenseCategory');if(!selected.includes(category))document.querySelector(`input[name="expenseCategory"][value="${CSS.escape(category)}"]`).checked=true;renderExpenseTags([...document.querySelectorAll('input[name="expenseTag"]:checked')].map(x=>x.value).concat(name));if(!document.querySelector('#editExpenseModal').hidden){const item=expenses.find(x=>x.id===Number(document.querySelector('#editExpenseId').value));renderEditTags(item?.tags||[])}};
