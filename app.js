...
function renderTagOptions(id,name,categories,selected=[]){const tags=tagsFor(categories),el=document.querySelector(id);el.innerHTML=tags.length?tags.map(tag=>`<label class="tag-option"><input type="checkbox" name="${name}" value="${esc(tag)}" ${selected.includes(tag)?'checked':''}><span>#${esc(tag)}</span></label>`).join(''):'<small class="tag-empty">태그 추가로 세부 분류를 만들어 보세요.</small>'}
