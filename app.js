...
function renderComparison(){const totals=new Map();expenses.forEach(x=>{const date=new Date(x.createdAt),key=`${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}`;totals.set(key,(totals.get(key)||0)+x.baseAmount)});const keys=[...totals.keys()].sort().slice(-6),max=Math.max(...keys.map(key=>totals.get(key)),1);document.querySelector('#reportComparisonChart').innerHTML=keys.length?`<div class="comparison-chart">${keys.map(key=>{const [year,month]=key.split('-');const value=totals.get(key);return `<div class="comparison-column"><span class="comparison-value">${format(value)}</span><i class="comparison-bar" style="height:${Math.max(value/max*100,3)}%"></i><span class="comparison-label">${Number(month)}월</span></div>`}).join('')}</div>`:'<div class="empty-state">비교할 월별 기록이 없어요.</div>'}
