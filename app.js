...
function tagGroups(items){const groups=new Map();items.forEach(item=>{const tags=item.tags||[];tags.forEach(tag=>{const current=groups.get(tag)||{name:tag,color:tagColor(tag,item.categories),value:0};current.value+=item.baseAmount/tags.length;groups.set(tag,current)})});return [...groups.values()].filter(group=>group.value).sort((a,b)=>b.value-a.value)}
function renderTagDonut(id,items){const groups=tagGroups(items),el=document.querySelector(id);if(!groups.length){el.innerHTML='<div class="empty-state">태그가 달린 지출을 추가하면 비율을 보여드려요.</div>';return}const total=groups.reduce((sum,group)=>sum+group.value,0);let progress=0,gradient=groups.map(group=>{const start=progress;progress+=group.value/total*100;return `${group.color} ${start}% ${progress}%`}).join(',');el.innerHTML=`<div class="tag-donut-chart"><div class="tag-donut" style="background:conic-gradient(${gradient})"></div><div class="tag-donut-legend">${groups.map(group=>`<div><span><i style="background:${group.color}"></i>#${esc(group.name)}</span><strong>${format(group.value)}</strong></div>`).join('')}</div></div>`}
...
document.querySelector('#viewTagSpend').onclick=()=>{const items=expenses.filter(item=>{const date=new Date(item.createdAt);return date.getFullYear()===selectedMonth.getFullYear()&&date.getMonth()===selectedMonth.getMonth()});renderTagDonut('#dashboardTagChart',items);document.querySelector('#tagSpendModal').hidden=false};
