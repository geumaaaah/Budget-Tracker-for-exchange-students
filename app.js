...
document.querySelector('#addTag').onclick=()=>{document.querySelector('#tagCategorySelect').innerHTML=allCategories().map(category=>`<option value="${esc(category)}">${esc(category)}</option>`).join('');const selected=selectedCategories('expenseCategory')[0];if(selected)document.querySelector('#tagCategorySelect').value=selected;document.querySelector('#tagName').value='';setTagNamePlaceholder();setTagColor('#ed7248');document.querySelector('#tagModal').hidden=false;document.querySelector('#tagName').focus()};
