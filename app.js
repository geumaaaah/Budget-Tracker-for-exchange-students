const dataKey='wander-wallet-expenses',baseKey='wander-wallet-base-currency',legacyKey='wander-wallet-currency',rateCacheKey='wander-wallet-rate-cache',budgetKey='wander-wallet-budget',customCategoryKey='wander-wallet-custom-categories',categoryOrderKey='wander-wallet-category-order',tagKey='wander-wallet-category-tags',onboardingKey='wander-wallet-onboarding-complete',travelCurrencyKey='wander-wallet-travel-currency';
...
let expenses=JSON.parse(localStorage.getItem(dataKey)||'[]'),rates=null,rateDate='',rateRequest=null,budget=Number(localStorage.getItem(budgetKey))||0,customCategories=JSON.parse(localStorage.getItem(customCategoryKey)||'[]'),categoryOrder=JSON.parse(localStorage.getItem(categoryOrderKey)||'null')||[...defaultCategories,...customCategories],categoryTags=JSON.parse(localStorage.getItem(tagKey)||'{}'),exchangeRequestId=0,pendingExpense=null;
...
function renderTagOptions(id,name,categories,selected=[]){const tags=tagsFor(categories),el=document.querySelector(id);el.innerHTML=tags.map(tag=>`<label class="tag-option"><input type="checkbox" name="${name}" value="${esc(tag)}" ${selected.includes(tag)?'checked':''}><span>#${esc(tag)}</span></label>`).join('')}
...
renderCategories()};
document.querySelector('#addTag').onclick=()=>{document.querySelector('#tagCategorySelect').innerHTML=allCategories().map(category=>`<option value="${esc(category)}">${esc(category)}</option>`).join('');const selected=selectedCategories('expenseCategory')[0];if(selected)document.querySelector('#tagCategorySelect').value=selected;document.querySelector('#tagName').value='';document.querySelector('#tagModal').hidden=false;document.querySelector('#tagName').focus()};
document.querySelector('#closeTagModal').onclick=()=>document.querySelector('#tagModal').hidden=true;
document.querySelector('#tagForm').onsubmit=e=>{e.preventDefault();const category=document.querySelector('#tagCategorySelect').value,name=document.querySelector('#tagName').value.trim();if(!name)return;if((categoryTags[category]||[]).includes(name))return alert('이미 추가된 태그예요.');categoryTags={...categoryTags,[category]:[...(categoryTags[category]||[]),name]};localStorage.setItem(tagKey,JSON.stringify(categoryTags));document.querySelector('#tagModal').hidden=true;const selected=selectedCategories('expenseCategory');if(!selected.includes(category))document.querySelector(`input[name="expenseCategory"][value="${CSS.escape(category)}"]`).checked=true;renderExpenseTags([...document.querySelectorAll('input[name="expenseTag"]:checked')].map(x=>x.value).concat(name));if(!document.querySelector('#editExpenseModal').hidden){const item=expenses.find(x=>x.id===Number(document.querySelector('#editExpenseId').value));renderEditTags(item?.tags||[])}};
