...
function moveCategory(fromName,toName){const from=categoryOrder.indexOf(fromName),to=categoryOrder.indexOf(toName);if(from<0||to<0||from===to)return;categoryOrder.splice(from,1);categoryOrder.splice(to,0,fromName);localStorage.setItem(categoryOrderKey,JSON.stringify(categoryOrder));renderCategories();render()}
categoryOptions.addEventListener('drop',e=>{e.preventDefault();const target=e.target.closest('.category-option');if(!target||!draggedCategory)return;moveCategory(draggedCategory,target.dataset.category);draggedCategory=''});
let touchCategory=null,suppressCategoryClick=false;
categoryOptions.addEventListener('pointerdown',e=>{if(e.pointerType!=='touch')return;const item=e.target.closest('.category-option');if(!item)return;touchCategory={item,name:item.dataset.category,startX:e.clientX,startY:e.clientY,dragging:false,pointerId:e.pointerId};categoryOptions.setPointerCapture?.(e.pointerId)});
categoryOptions.addEventListener('pointermove',e=>{if(!touchCategory||e.pointerId!==touchCategory.pointerId)return;const moved=Math.hypot(e.clientX-touchCategory.startX,e.clientY-touchCategory.startY)>7;if(!touchCategory.dragging&&!moved)return;touchCategory.dragging=true;e.preventDefault();touchCategory.item.classList.add('touch-dragging');const target=document.elementFromPoint(e.clientX,e.clientY)?.closest('.category-option');categoryOptions.querySelectorAll('.touch-drag-over').forEach(item=>item.classList.remove('touch-drag-over'));if(target&&target.dataset.category!==touchCategory.name)target.classList.add('touch-drag-over')});
function finishTouchCategory(e){if(!touchCategory||e.pointerId!==touchCategory.pointerId)return;const {item,name,dragging}=touchCategory,target=dragging?document.elementFromPoint(e.clientX,e.clientY)?.closest('.category-option'):null;item.classList.remove('touch-dragging');categoryOptions.querySelectorAll('.touch-drag-over').forEach(option=>option.classList.remove('touch-drag-over'));if(dragging){suppressCategoryClick=true;if(target&&target.dataset.category!==name)moveCategory(name,target.dataset.category);setTimeout(()=>{suppressCategoryClick=false},0)}touchCategory=null}
categoryOptions.addEventListener('pointerup',finishTouchCategory);
categoryOptions.addEventListener('pointercancel',finishTouchCategory);
categoryOptions.addEventListener('click',e=>{if(!suppressCategoryClick)return;e.preventDefault();e.stopPropagation()},true);
...
document.querySelector('#expenseForm').onsubmit=e=>{e.preventDefault();if(!baseCurrency)return openModal();const amount=Number(document.querySelector('#expenseAmount').value),inputCurrency=document.querySelector('#inputCurrency').value,selected=[...document.querySelectorAll('input[name="expenseCategory"]:checked')].map(x=>x.value),tags=[...document.querySelectorAll('input[name="expenseTag"]:checked')].map(x=>x.value),selectedDate=new Date(`${expenseDate.value}T12:00:00`);if(!selected.length)return alert('지출 항목을 하나 이상 선택해 주세요.');const expense={name:document.querySelector('#expenseName').value.trim(),amount,inputCurrency,categories:selected,tags,date:`${selectedDate.getMonth()+1}/${selectedDate.getDate()}`,createdAt:selectedDate.toISOString()};openRateModal(expense)};
